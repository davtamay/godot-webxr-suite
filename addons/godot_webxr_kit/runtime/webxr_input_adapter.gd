...
## WebXR input source: interface-level selectstart/selectend signals resolved to
## handedness, controller aim poses from XRController3D, and the standard
## XRHandTracker hand-ray/pinch fallback (Godot's WebXRInterface already fills
## XRHandTracker on every platform, so this stays engine-standard - no browser
## JS hand bridge). Inert outside web exports.
